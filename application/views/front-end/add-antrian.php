						...
						<div class="col-md-12">
							...
							<div class="col-md-6">
								<div class="featured-box" align="left">
									<div class="box-content">
										<div class="row">
											<h3>Jadwal Antrian Dokter</h3>
											<?php 
												for ($i=0; $i < $vaDokter['max'] ; $i++) { 
												$jamBuka = explode(':',$vaDokter['jam_buka']);
												$jam = $jamBuka[0];
												$menit = $jamBuka[1];
												$date = date_create($vaDokter['jam_buka']);
												$jamHarus = $vaDokter['menit_pasien']*$i;
												date_add($date, date_interval_create_from_date_string(''.$jamHarus.' Minutes'));
											?>
											
											<a href="<?=base_url()?>website/isi_antrian/<?php echo date_format($date, 'H:i');?>/<?=$vaDokter['id_prakter']?>" class="btn btn-primary mt-xl">Pilih Jam Antrian Anda : <?php echo date_format($date, 'H:i'); ?></a>
